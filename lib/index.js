...
// _defineProperty() is transpiled from query: { [idKey]: { $in: params.ids, }, }
// from ra-data-feathers/src/restClient.js
...
class Service extends AdapterService {
  ...
  _remove (id, params = {}) {
    ...
    // In multi remove query que expect something like this: { _id: { '$in': [ '5e4cb5e3dd27050beb6eed2b' ] } }
    // The problem is that the ids are not found with the original query ( ids in quotations marks ).
    if ( query._id && query._id.$in ) {
      let ids = query._id.$in;
      let newIds = [];
      ids.forEach( elem => {
        if ( ObjectID.isValid(elem) ) {
          newIds.push(new ObjectID(elem) );
        }
      });
      // _defineProperty() is transpiled from the original code: { [idKey]: { $in: params.ids, }, }
      // from ra-data-feathers/src/restClient.js
      // In theory, we could do this at ra-data-feathers/restClient.js but, it is not only for mongodb.
      // So, I think this is the best place to fix the problem.
      let newQuery =_defineProperty( {}, '_id', { $in: newIds});
      query = newQuery;
    }
    ...
    const findParams = Object.assign({}, params, {
      paginate: false,
      // query params.query --> I think this is also an error, why not use query instead of params.query, which is also modified some lines above.
      query: query // We use the newQuery, copied to query variable @263.
    });
